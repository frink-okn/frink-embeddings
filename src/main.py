...
def english_with_types(
    ...
    types = [(t or "").strip() for t in (type_terms or []) if (t or "").strip()]
    parents = [(p or "").strip() for p in (parent_terms or []) if (p or "").strip()]
...
def write_tsv(
    records: Iterator[Tuple[URIRef, str, str, torch.Tensor]], output: str
) -> None:
...
def write_json(
    records: Iterator[Tuple[URIRef, str, str, torch.Tensor]],
    output: str,
    graph_name: str,
) -> None:
    idx = 0
    ...
    for pred in lookup_label.predicates:  # type: ignore
        ...
        for value in values:
            if isinstance(value, Literal):
                if value.language and (
                    value.language == "en" or value.language.startswith("en-")
                ):
...
def representation_for_subject(
    subject: URIRef,
    data: dict[URIRef, list[Identifier]],
    doc: HDTDocument,
    ...
    label_and_types = english_with_types(
        subject_label,
        [v for (k, v) in labeled_types],
        [v for (k, v) in labeled_parents],
    )
    extra_labels = []
    descriptions = []
...
def stream_by_subject(
    doc: HDTDocument, conf: dict[str, list[str]]
) -> Iterator[Tuple[URIRef, Dict[URIRef, list[Identifier]]]]:
    """
    ...
    for subj, group in itertools.groupby(triples, key=lambda t: t[0]):
        ...
        for _, p, o in group:
            if (
                p in keep_predicates
                and not o.startswith("_:")
                and not o == subj
                and not o.startswith(str(RDF))
                and not o.startswith(str(RDFS))
                and not o.startswith(str(OWL))
            ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=pathlib.Path,
        help="An hdt file from an rdf graph",
    )
    parser.add_argument(
        "-c",
        "--conf",
        required=True,
        type=pathlib.Path,
        help="The yaml file for configuration",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        help="The ourput file path or URL for the Qdrant endpoint",
    )
    parser.add_argument(
        "-n",
        "--collection-name",
    ...
    parser.add_argument(
        "-g", "--graph-name", required=False, help="The name of the graph"
    )
    parser.add_argument(
        "-m",
        "--mode",
        type=lambda s: s.lower(),
        choices=["tsv", "json", "qdrant"],
        required=True,
        help="Output mode: TSV, JSON, or Qdrant",
    )
